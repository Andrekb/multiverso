<x-app-layout>
    <x-slot name="header">
        <h2>lojas</h2>
        <x-secondary-button class="bg-secondary" :href="route('lojas.create')">criar</x-secondary-button>
    </x-slot>
    <div class="flex-column list-content">
        @if (session('status') === 'store-created')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="success">Loja cadastrada com sucesso</p>
        @elseif(session('status') === 'store-updated')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="success">Loja alterada com sucesso</p>
        @elseif(session('status') === 'store-not-destroyed')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="not-success">Loja possui produtos vinculados, não é possível excluir</p>
        @elseif(session('status') === 'store-destroyed')
            <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 2000)" class="success">Loja deletada com sucesso</p>
        @endif
        ...
        <x-line-link>
            ...
            <div class="crud-buttons flex-row">
                ...
                <a class="primary-button bg-secondary" href="{{ route('lojas.edit', $store->id) }}">
                ...
                <form method="post" action="{{ route('lojas.destroy', $store->id) }}" class="p-6">                        
                    @csrf
                    @method('delete')
